...
def upscale_image_with_opencv(image_path, scale_factor):
    """Upscale the image using OpenCV by the given scale factor."""
    image = cv2.imread(image_path)
    height, width = image.shape[:2]
    new_size = (int(width * scale_factor), int(height * scale_factor))
    upscaled_image = cv2.resize(image, new_size, interpolation=cv2.INTER_LANCZOS4)
    temp_dir = tempfile.gettempdir()
    upscaled_image_path = os.path.join(temp_dir, f"upscaled_{os.path.basename(image_path)}")
...
def switch_printer(printer_model, frame_type):
    ...
    try:
        # Upscale the image using OpenCV before printing
        upscaled_image_path = upscale_image_with_opencv(file_path, 4.0)  # 2.0은 스케일 팩터, 필요에 따라 조정 가능

        # Print the upscaled image file
        print_image_with_rundll32(upscaled_image_path, device_id, frame_type, num_copies)
    except Exception as e:
        logging.error(f"Error processing print job: {e}")
        return jsonify({'error': str(e)}), 500
    finally:
